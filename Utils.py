...
def accuracy(output, target, topk=(1,), is_show=False):
...
def Collect_Labeled_Image_Indexs(entropy_threshold, indexs, preds_, target):
    pre = torch.softmax(preds_[0], dim=1)
    maxsc, fakelabel = torch.max(pre, dim=-1)
    preds = [np.argmax(pred.cpu().data.numpy(), axis=1) for pred in preds_]# shape is (7, batchSize,)
    target = target.cpu().data.numpy()
    softmaxs = [nn.Softmax(dim=1)(preds_[i]) for i in range(7)]    # shape是（7, batchSize,）
    entropys = [torch.sum(-softmaxs[i] * torch.log(softmaxs[i] + 1e-5), dim=1) for i in range(7)] #计算七个分类器的信息熵, shape是(7, batchSize, 1)
    ...
    for img_index in range(len(preds[0])):
        # if preds[0][img_index] == preds[1][img_index] == preds[2][img_index] == preds[3][img_index] == preds[4][img_index] == preds[5][img_index]:
        # if maxsc[img_index] > 0.95:
...
def Collect_Labeled_Image_Indexs2(entropy_threshold, indexs, preds_, target):
    preds = np.argmax(preds_.cpu().data.numpy(), axis=1) # shape is (batchSize)
    target = target.cpu().data.numpy()
    softmaxs = nn.Softmax(dim=1)(preds_) # shape是（batchSize,）
    entropys = torch.sum(-softmaxs * torch.log(softmaxs + 1e-5), dim=1)  # 计算分类器的信息熵, shape是(batchSize, 1)

    collect_indexs, fake_labels, true_labels = [], [], []

    for img_index in range(len(preds)):
        collect_indexs.append(indexs[img_index])
        fake_labels.append(preds[img_index])
        true_labels.append(target[img_index])
...
def BuildLabeledDataloader(args, dataloader, init_dataset_data, model, flag='train'):
    ...
    collect_num = 0
    category_num_true = [0,0,0,0,0,0,0]
    category_num_fake = [0,0,0,0,0,0,0]
    category_num_confi = [0,0,0,0,0,0,0]
    category_num_prop = [0,0,0,0,0,0,0]
    eps = 1e-6
    for batch_index, (indexs, input, landmark, target, img_aug) in enumerate(bar):
        input, landmark, target = input.cuda(), landmark.cuda(), target.cuda()

        # Forward Propagation
        with torch.no_grad():
            features, preds, _ = model(input, landmark, target, img_aug,args, is_eval=True)
    ...
    proportion = len(data_labels) / (len(dataloader) * args.train_batch_size)
    confidence = np.sum(np.array(data_labels) == np.array(target_labels)) / len(data_labels)
    for i in range(len(data_labels)):
        if data_labels[i] == target_labels[i]:
            category_num_true[data_labels[i]] += 1
        category_num_fake[data_labels[i]] += 1
    for i in range(7):
        if category_num_fake[i] != 0:
            category_num_confi[i] = category_num_true[i] / category_num_fake[i]
        else:
            category_num_confi[i] = 0

    for i in range(7):
        category_num_prop[i] = category_num_fake[i] / collect_num

    print(category_num_true[0], category_num_true[1], category_num_true[2], category_num_true[3], category_num_true[4], category_num_true[5], category_num_true[6])
    print(category_num_fake[0], category_num_fake[1], category_num_fake[2], category_num_fake[3], category_num_fake[4], category_num_fake[5], category_num_fake[6])
    print(category_num_confi[0], category_num_confi[1], category_num_confi[2], category_num_confi[3], category_num_confi[4], category_num_confi[5], category_num_confi[6])
    print(category_num_prop[0], category_num_prop[1], category_num_prop[2], category_num_prop[3],
          category_num_prop[4], category_num_prop[5], category_num_prop[6])
    print(f"collect {collect_num} images from target domain to train. proportion is {proportion}, confidence is {confidence}")
    ...
    # DataSet
    data_set = MyDataset2(data_imgs, data_labels, data_bboxs, data_landmarks, target_labels, flag, trans) # trans是tansformer处理，data_set是一个图片集，包括裁剪后的人脸图片，图片的五个关键点，表情标签
    if flag == 'train':
    ...
    return data_loader, confidence, proportion, collect_num, category_num_confi, category_num_prop


def BuildLabeledDataloader2(args, dataloader, init_dataset_data, model, flag='train'):
    """Test."""
    model.eval()
    bar = tqdm(dataloader)
    data_imgs, data_labels, data_bboxs, data_landmarks, target_labels = [], [], [], [], []
    ...
    for batch_index, (indexs, input, landmark, target, img_aug) in enumerate(bar):
        ...
        # Forward Propagation
        with torch.no_grad():
            features, preds, _ = model(input, landmark, target, img_aug, args, class_acc ,is_eval=True)

        collect_list_index, fake_labels, true_labels = Collect_Labeled_Image_Indexs2(0.19459, indexs, preds, target)
        data_labels += fake_labels
        target_labels += true_labels
    ...
    print(
        f"collect {collect_num} images from target domain to train. probility is {probility}, confidence is {confidence}")
    ...
    # DataSet
    data_set = MyDataset2(data_imgs, data_labels, data_bboxs, data_landmarks, target_labels, flag,
                          trans)  # trans是tansformer处理，data_set是一个图片集，包括裁剪后的人脸图片，图片的五个关键点，表情标签
    if flag == 'train':
        data_loader = data.DataLoader(dataset=data_set, batch_size=args.train_batch_size, shuffle=True, num_workers=8,
                                      drop_last=True)
    elif flag == 'test':
        data_loader = data.DataLoader(dataset=data_set, batch_size=args.test_batch_size, shuffle=False, num_workers=8,
                                      drop_last=False)

    return data_loader, confidence, probility, collect_num


def generate_candiate_labels(train_labels):
...
def Show_OnlyAccuracy2(accuracys):
    accs = 0 # 预设7个分类器的分类准确率为0
    for class_id in range(len(accuracys)):
            accs += accuracys[class_id].avg
    return accs / len(accuracys)
...
def Initialize_Mean_Cluster(args, model, useClassify=True,source_data_loader=None,target_data_loader=None):
    ...
    class_acc = None
    for step, (_, input, landmark, label, aug) in enumerate(source_data_bar):
        input, landmark, label = input.cuda(), landmark.cuda(), label.cuda()
        with torch.no_grad():
            #! m21-11-12，注释
            # feature, output, loc_output = model(input, landmark)
            #@ m21-11-11, 新的model调用
            features, preds, tgt = model(input, landmark, label, aug, args, class_acc, is_eval=True)
    ...
    for step, (_, input, landmark, label, aug) in enumerate(target_data_bar):
        input, landmark, label = input.cuda(), landmark.cuda(), label.cuda()
        with torch.no_grad():
            #! m21-11-12，注释
            # feature, output, loc_output = model(input, landmark)
...
def Visualization(figName, model, dataloader=None, useClassify=True, domain='Source'):
    ...
    for i in range(7):
        if domain == 'Source':
            Feature.append(model.prototypes[i].cpu().data.numpy()) # 把七个聚类点先存进Features列表中
        elif domain == 'Target':
            Feature.append(model.prototypes_tag[i].cpu().data.numpy())
        # Label.append(np.array(i+7))
    Label.append(np.array([7+i for i in range(7)])) # 先预设7个聚类点，这7个点对应Feature里边的前7个点，作为聚类点

    # Get Feature and Label
    for step, (_,input, landmark, label,_) in enumerate(dataloader):
        '''
        参数解释
        ...
        input, landmark, label = input.cuda(), landmark.cuda(), label.cuda()
        with torch.no_grad():
            # feature, output, loc_output = model.encoder_q(input, landmark, useClassify, domain)
            feature, _, _, _ = model.encoder_q(input, landmark, useClassify, domain)
            '''
            feature: batch * 384
    ...
    colors = {0:'red', 1:'blue', 2:'olive',  3:'green',  4:'orange',  5:'purple',  6:'darkslategray', 7:'black', 8:'lime', 9:'deeppink', 10:'cyan', 11:'peru', 12:'gold', 13:'yellow'}

    labels = {0:'Surprised', 1:'Fear', 2:'Disgust',  3:'Happy',  4:'Sad',  5:'Angry',  6:'Neutral', 7:'C-Surprised', 8:'C-Fear', 9:'C-Disgust', 10:'C-Happy', 11:'C-Sad', 12:'C-Angry', 13:'C-Neutral'}
    ...
    scatter = plt.scatter(data_norm[Label==7][:,0], data_norm[Label==7][:,1], c=colors[7], s=30, label=labels[7], marker='o', alpha=1)
    scatter = plt.scatter(data_norm[Label == 8][:,0], data_norm[Label == 8][:,1], c=colors[8], s=30, label=labels[8],
                          marker='o', alpha=1)
    scatter = plt.scatter(data_norm[Label == 9][:,0], data_norm[Label == 9][:,1], c=colors[9], s=30, label=labels[9],
                          marker='o', alpha=1)
    scatter = plt.scatter(data_norm[Label == 10][:,0], data_norm[Label == 10][:,1], c=colors[10], s=30, label=labels[10],
                          marker='o', alpha=1)
    scatter = plt.scatter(data_norm[Label == 11][:,0], data_norm[Label == 11][:,1], c=colors[11], s=30, label=labels[11],
                          marker='o', alpha=1)
    scatter = plt.scatter(data_norm[Label == 12][:,0], data_norm[Label == 12][:,1], c=colors[12], s=30, label=labels[12],
                          marker='o', alpha=1)
    scatter = plt.scatter(data_norm[Label == 13][:,0], data_norm[Label == 13][:,1], c=colors[13], s=30, label=labels[13],
                          marker='o', alpha=1)

    box = ax.get_position()
    ax.set_position([box.x0, box.y0, box.width * 0.8, box.height * 0.8])
    
    plt.legend(handles=[mpatches.Patch(color=colors[i], label="{:s}".format(labels[i]) ) for i in range(14)],
               loc='upper left',
            #    prop = {'size':8},
...
def makeFolder(args):
    ...
    for fileName in ['AdversarialNetwork.py', 'Dataset.py', 'demo.py', 'getPreTrainedModel_ResNet.py', 'GraphConvolutionNetwork.py', 'Loss.py', 'Model.py',
        'ResNet.py', 'TrainOnSourceDomain.py', 'TrainOnSourceDomain.sh', 'TransferToTargetDomain.py', 'TransferToTargetDomain.sh', 'Utils.py', 'model.py']:
        try:
...
def Visualize_Collection_Pseudo_Labels_Category_ClassWise(classwises):
    ...
    df = pd.DataFrame(data, columns=["Expression", "Global", "G-Local"])
    ax = df.plot(x="Expression", y=["Global", "G-Local"], kind="bar",
                 figsize=(9, 8), ax=ax, title='category thres of each classifiers', grid=True, rot=360)
...
def Draw_PL_Distribute_Bar(category_confidence, category_proportion, domain,
                           xaxis=['Surprised', 'Fear', 'Disgust', 'Happy', 'Sad', 'Angry', 'Neutral']):
    fig, ax = plt.subplots()
    ...
    df.plot(x="Expression", y=["Confidence", "Proportion"], kind="bar", figsize=(9, 8), ax=ax,
            title='Category Info' + f'({domain})', grid=True, sharex=True, rot=360)

    return fig


def Visualize_Pseudo_Labels_Generator_two(args, epoch, writer, confidence, proportion, category_confidence, category_confidence_2):
    #@ The proportion and confidence of the number of target domains with pseudo labels used for training
    writer.add_scalars('Pseudo_Labels_Generator/Labeled_Prop_Confi', {'confidence': confidence, 'probility': proportion}, epoch)

    #@ Plot the proportion and confidence of pseudo labels on each category in a bar chart
    fig = Draw_PL_Distribute_Bar_two(category_confidence, category_confidence_2, args.targetDataset)
    writer.add_figure('Pseudo_Labels_Generator/Category_Confidence_Proportion', fig, global_step=epoch, close=True)

def Draw_PL_Distribute_Bar_two(category_confidence_1, category_confidence_2, domain,
                           xaxis=['Surprised', 'Fear', 'Disgust', 'Happy', 'Sad', 'Angry', 'Neutral']):
    fig, ax = plt.subplots()

    data = []
    for index in range(len(category_confidence_1)):
        data.append([])
        data[index].append(xaxis[index])
        data[index].append(category_confidence_1[index])
        data[index].append(category_confidence_2[index])

    df = pd.DataFrame(data, columns=["Expression", "Confidence", "Confidence_wo_RW"])
    df.plot(x="Expression", y=["Confidence", "Confidence_wo_RW"], kind="bar", figsize=(9, 8), ax=ax,
            title='Category Info' + f'({domain})', grid=True, sharex=True, rot=360)
